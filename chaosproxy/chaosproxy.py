...
class DummyResponse:
    ...
    def __init__(self, req_id, stream=None):
        ...
        self.stream = stream
        self.code = 500
        self.headers = DummyResponse.DummyHeader()
...
class ChaosProxy:
    ...
    class Proxy(SimpleHTTPServer.SimpleHTTPRequestHandler):
        ...
        def __do_proxy(self):
            ...
            new_headers['ChaosProxyRequestId'] = req_id
            try:
            ...
            logging.debug('%s - Request processed', req_id)
        ...
        def __do_request(self, url, body, headers, req_id, request_delay, response_delay, request_drop, response_drop):
            ...
            try:
                ...
                if request_drop:
                    logging.debug('%s - Dropped request to [%s]', req_id, url)
            ...
            if response_drop:
                logging.debug('%s - Dropped response from [%s]', req_id, url)
                return DummyResponse(req_id, response)  # drop response
            if response_delay:
            ...
            self.headers['ChaosProxyRequestId'] = req_id
